...
describe("persisted chat image artifact recovery", () => {
  ...
  it.each(["read", "response"] as const)("rechecks sharing after the %s await", async (phase) => {
    await withOpenClawTestState({ scenario: "minimal" }, async () => {
      ...
      expect(await invoke("artifacts.download", { artifactId }, client)).toMatchObject({
        ok: true,
      });
      ...
      const revoke = () =>
        upsertSessionEntryCore(scope, { ...entry, updatedAt: 2, visibility: "draft" });
      const spy =
        phase === "read"
          ? vi
      ...
      try {
        const denied = await invoke("artifacts.download", { artifactId }, client);
